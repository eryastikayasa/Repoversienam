...
static bool ensure_rx_worker(void)
{
    ...
    s_rx_free_queue = xQueueCreateStatic(
        RX_BUFFER_COUNT, sizeof(char *),
        reinterpret_cast<uint8_t *>(s_rx_free_storage), &s_rx_free_queue_storage);
    s_rx_ready_queue = xQueueCreateStatic(
        RX_BUFFER_COUNT, sizeof(rx_item_t),
        reinterpret_cast<uint8_t *>(s_rx_ready_storage), &s_rx_ready_queue_storage);
    ...
    if (xTaskCreate(
            [](void *) {
                ESP_LOGI(TAG, "WS_RX: worker START buffers=%u size=%uKB",
                         (unsigned)RX_BUFFER_COUNT, (unsigned)(RX_MAX_PAYLOAD / 1024U));
                for (;;) {
                    rx_item_t item = {};
                    ...
                    if (item.generation != current_generation) {
                        ...
                        ESP_LOGI(TAG, "WS_RX: complete message len=%u generation=%lu",
                                 (unsigned)item.len, (unsigned long)item.generation);
                        log_hex("worker first bytes:",
                                reinterpret_cast<const uint8_t *>(item.buffer), item.len);
                        if (item.len > 16U) {
                            log_hex("worker last bytes:",
                                    reinterpret_cast<const uint8_t *>(item.buffer + item.len - 16U), 16U);
                        }
                        (void)gemini_protocol_process_message(
                            item.buffer, item.len, item.generation);
                    }

                    if (xQueueSend(s_rx_free_queue, &item.buffer, portMAX_DELAY) != pdPASS) {
...
static void handle_data_event(esp_websocket_event_data_t *event)
{
    ...
    ESP_LOGI(TAG,
             "WS_RX: DATA raw_opcode=0x%02X opcode=0x%02X fin=%d payload_len=%u payload_offset=%u data_len=%u",
             raw_opcode, opcode, fin ? 1 : 0, (unsigned)payload_len,
             (unsigned)payload_offset, (unsigned)data_len);
    log_hex("first bytes:", reinterpret_cast<const uint8_t *>(event->data_ptr), data_len);
    if (data_len > 16U) {
        log_hex("last bytes:", reinterpret_cast<const uint8_t *>(event->data_ptr + data_len - 16U), 16U);
    }
    ...
    if (payload_offset == 0U) {
        ...
        if (opcode != 0x01U && opcode != 0x02U) {
            ESP_LOGW(TAG, "WS_RX: first fragment must be TEXT/BINARY, opcode=0x%02X", opcode);
            return;
        }
    ...
    if (!s_rx_text_message) {
        ESP_LOGW(TAG, "WS_RX: BINARY frame is not a Gemini JSON message; dropped");
        reset_rx();
        return;
    }
    ...
    if (fin && s_rx_received != s_rx_expected) {
        ESP_LOGW(TAG, "WS_RX: FIN before payload complete received=%u expected=%u",
                 (unsigned)s_rx_received, (unsigned)s_rx_expected);
        reset_rx();
        return;
    }
    if (!fin && s_rx_received == s_rx_expected) {
        ESP_LOGI(TAG, "WS_RX: payload bytes complete but FIN=0; waiting continuation");
        return;
    }
    ...
    if (xQueueSend(s_rx_ready_queue, &item, 0) != pdPASS) {
        ...
        if (xQueueSend(s_rx_free_queue, &item.buffer, 0) != pdPASS)
...
void websocket_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case WEBSOCKET_EVENT_CONNECTED:
        websocket_transport_event_connected();
        websocket_event_reset();
        (void)ensure_rx_worker();
        audio_engine_notify(AUDIO_ENGINE_EVENT_GENERATION_CHANGED, websocket_transport_generation());
        if (!gemini_protocol_on_connected()) {
    ...
    case WEBSOCKET_EVENT_DISCONNECTED:
        websocket_audio_on_disconnected();
        audio_engine_stop_input_session();
        gemini_protocol_on_disconnected();
        reset_rx();
        websocket_transport_event_disconnected();
        break;
...
{
    return gemini_protocol_setup_complete();
}
