...
static void log_capture_window_if_ready()
{
    ...
    ESP_LOGI("AUDIO_HAL",
             "AUDIO_CAPTURE_PROFILE: count=%llu mic_read_avg_us=%llu mic_read_max_us=%llu i2s_read_avg_us=%llu i2s_read_max_us=%llu aec_avg_us=%llu aec_max_us=%llu queue_push_avg_us=%llu queue_push_max_us=%llu",
             (unsigned long long)count,
             (unsigned long long)(load_us(&s_capture_read_us) / count),
             (unsigned long long)load_us(&s_capture_read_max_us),
             (unsigned long long)(load_us(&s_i2s_read_us) / count),
             (unsigned long long)load_us(&s_i2s_read_max_us),
             (unsigned long long)(load_us(&s_aec_us) / count),
             (unsigned long long)load_us(&s_aec_max_us),
             (unsigned long long)(load_us(&s_queue_push_us) / count),
             (unsigned long long)load_us(&s_queue_push_max_us));
...
extern "C" BaseType_t __wrap_xQueueGenericSend(QueueHandle_t queue, const void *item, TickType_t ticks_to_wait, BaseType_t copy_position)
{
    ...
    const int64_t start = esp_timer_get_time();
    const BaseType_t ret = __real_xQueueGenericSend(queue, item, ticks_to_wait, copy_position);
    const uint64_t elapsed = (uint64_t)(esp_timer_get_time() - start);
    add_us(&s_queue_push_us, elapsed);
    update_max(&s_queue_push_max_us, elapsed);
    return ret;
}
